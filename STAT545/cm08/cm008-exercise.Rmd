...
gapminder %>% 
  mutate(
    lifeExp = if_else(country == "Canada" & year == 1952, 70, lifeExp)
  ) %>% 
...
gapminder %>% 
  mutate(
    age = if_else(lifeExp > 60, "high", "low")
    ) %>% 
...
```{r}
continent_max_drop <- gapminder %>% 
  group_by(continent, country) %>% 
  mutate(
    drop = lag(lifeExp) - lifeExp
  ) %>% 
  filter(!is.na(drop)) %>% 
  summarize(
    max_drop_countries = max(drop)
  ) %>% 
  group_by(continent) %>% 
  summarize(
    max_drop = max(max_drop_countries)
  )

country_max_drop <- gapminder %>% 
  group_by(continent, country) %>% 
  mutate(
    drop = lag(lifeExp) - lifeExp
  ) %>% 
...
left_join(x=continent_max_drop, y=country_max_drop, by="max_drop") %>% 
  select(country, continent, max_drop)
```
...
my_df <- gapminder %>% 
  mutate(
    cda_gdp = rep(cda, nlevels(country)),
    gdp = gdpPercap * pop,
    rel_gdp = gdp / cda_gdp
  )
